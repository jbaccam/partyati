...
for s in [-1,1]:
 trap=ellipsoid('Thick sloping trapezius','Torso',(s*1.35-.16,.18,10.05),(2.9,1.95,1.40),skin,20,12)
 trap.rotation_euler.y=math.radians(s*19)
...
for side,s in [('Right',-1),('Left',1)]:
 ...
 thumbSide=1 if side=='Right' else s
 tube('Opposing thick thumb',group,[(g-thumbSide*.59,-.03,.66),(g-thumbSide*.70,-.33,.35),(g-thumbSide*.48,-.49,.14)],[.26,.26,.19],skin,12)
 # Half-turn around the wrist axis: the palm comes over the shaft while
 # the wrist remains above it, rather than swinging the wrist below the haft.
 gripRotation=Matrix.Translation((g,0,0))@Matrix.Rotation(math.pi if side=='Left' else 0,4,'Z')@Matrix.Translation((-g,0,0))
 for o in set(bpy.data.objects)-new:o.matrix_world=hammerT@gripRotation@o.matrix_world
 wrists[side]=hammerT@gripRotation@Vector((g,.40,.90))
...
for side,s in [('Right',-1),('Left',1)]:
 ...
 # A short tapered wrist follows the forearm into the palm without a ball joint.
 connection=(elbow-wrist).normalized()
 tube('Tapered wrist joining the palm',side+'Hand',[wrist+connection*.22,wrist,wrist-connection*.34],[(.53,.54),(.52,.53),(.50,.49)],skin,20)
