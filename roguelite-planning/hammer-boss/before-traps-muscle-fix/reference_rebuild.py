...
loft('Continuous chest back and trapezius','Torso',[(4.5,0,.10,1.7,1.05),(5.2,0,-.10,2.16,1.50),(6.4,0,-.20,2.56,1.72),(7.6,0,.04,2.52,1.42),(8.6,0,.10,2.67,1.35),(9.2,0,.18,2.76,1.24),(9.65,-.08,.16,2.47,1.12),(10.0,-.12,.10,1.93,.99),(10.30,-.16,.02,1.35,.85),(10.47,-.17,-.08,.78,.66)],skin,40)
...
# Use the existing enemy's clean, full-resolution face tile directly.
family_uv(head,True)
...
for j in range(rows):
 ...
 for i in range(N):
  ...
  x=max(bx,cx)*math.cos(a)*1.027
  bellyY=-.71+by*math.sin(a);chestY=.10+cy*math.sin(a)
  y=min(bellyY,chestY)-.060 if math.sin(a)<0 else max(bellyY,chestY)+.095
  verts.append((x,y,z))
...
seat=loft('Continuous full trouser seat and waistband','Hips',[(3.08,0,.26,1.37,.80),(3.35,0,.40,1.77,1.14),(3.72,0,.43,2.00,1.42),(4.04,0,.37,2.03,1.50),(4.40,0,.17,1.97,1.32),(4.85,0,.03,1.89,1.14)],pants,40)
...
for side,s in [('Right',-1),('Left',1)]:
 ...
 tube('Continuous shaped deltoid and upper arm',side+'UpperArm',[shoulder+Vector((-s*.30,0,1.10)),shoulder+Vector((-s*.15,0,.82)),shoulder+Vector((0,0,.34)),shoulder,shoulder.lerp(elbow,.50),elbow+Vector((0,0,.15)),elbow-Vector((0,0,.20))],[(.22,.24),(.85,.85),(1.22,1.10),(1.18,1.10),(1.03,.99),(.81,.83),(.68,.71)],skin,28)
 tube('Continuous elbow forearm and wrist',side+'Forearm',[elbow+Vector((0,0,.24)),elbow,elbow.lerp(wrist,.26),elbow.lerp(wrist,.60),elbow.lerp(wrist,.86),wrist,(wrist+(wrist-elbow).normalized()*.08)],[(.68,.70),(.79,.82),(.94,.94),(.82,.81),(.61,.60),(.48,.48),(.46,.46)],skin,28)
