...
Air movement: Jumping and Freefall receive a 12% WalkSpeed boost, derived from the current resolved ground speed (default Brawler: 22 ground / 24.64 air). Landing or leaving those states restores ground speed. Stat changes during a jump retain the multiplier; repeated jumps do not stack it. The server applies it through CharacterService. Live playtesting is left to the user.

Movement tuning (September 21, second reference): base MoveSpeed is 24 studs/s, making the default Brawler 22 instead of 18. Class/item modifiers remain active. Player JumpHeight is 9.5 studs (previously 7.2) with UseJumpPower disabled; Workspace gravity is 150 (previously 196.2). CharacterService reapplies player movement on spawn/stat refresh. Gravity also affects other unanchored physics objects. Live playtesting is left to the user as requested.
...
Slot anchors follow the character's horizontal orientation at 0, 60, 120, 180, 240, and 300 degrees. A common radius uses stable root width, independently of animated limbs or weapon length. Client visuals follow those anchors with spring lag and may overlap the avatar during turns and jumps. See [weapon follow reference and validation](WEAPON_FOLLOW_REFERENCE.md). Weapons do not circulate. Each slot independently selects its nearest living tagged enemy with a clear line of sight, aims toward it, and attacks on its own server cooldown. Melee uses sampled model overlap plus distance and obstruction checks; ranged weapons use server rays, shotgun spread uses five rays, and blast weapons use bounded, obstruction-checked splash. Clients render shared attack poses, recoil, tracers/blasts, and confirmed damage numbers. Empty slots have no model or attack.
...
Tuning: 2.8-second cooldown (previously 1.5), 80 studs/second (increased from 55), 70-stud maximum travel, 9-stud explosion radius, up to 70 damage with linear falloff to 35% at the edge. Each living enemy can receive one damage application per blast. The server sweeps the entire traveled segment each frame, checks the path to the muzzle, resolves world/enemy impact, and checks blast line of sight. No Roblox Explosion physics or player damage is used. Rockets fly straight toward the acquired aim, with a visible modeled projectile, exhaust, and impact flash. No damage happens at launch. Misses expire without an impact explosion; wave disable and character removal cancel in-flight rockets.
...
## Current mob feedback (September 22)

The earlier blue/white cloud death effect is superseded by `MobImpactVisuals.luau`: curated outward white streaks and an irregular round red death burst from the user’s atlases. See `MOB_IMPACT_REFERENCE.md` for composition, asset selection and validation. Ordinary mob hits also use this module; environmental bullet impacts remain separate. The gold sheet is not used. Playtesting remains with the user.


## Randomized muzzle flashes � September 22, 2026

Glock, Draco and shotgun now use three compact top-row stars from the user's supplied gold sheet. These are separate selections from world-impact rows 2/4 and the white/red mob effects. Exact unchanged source, image ID and crop coordinates are in `assets/muzzle-fx-selection.json`. The existing uploaded atlas is reused; no image editing/upload was needed.

`MuzzleFlashVisuals` follows the rendered barrel after the weapon's render step. Flashes vary shape (no immediate repeats), rotation, scale and 55�85 ms lifetime. Glock is smaller than Draco; shotgun is largest. Pixel size is capped at 46, draw distance at 150 studs and active effects at 48. Billboards respect occlusion. Six initially pooled sprites retain their decoded images between shots; expired sprites become fully transparent and are reused. Each variant has a separate pool, avoiding crop changes during firing. Removing the visual container releases the pool and render binding.

A server-generated trigger timestamp travels on first-projectile packets, so shotgun pellets share one flash. Subsequent flight/contact/bounce packets cannot retrigger it. Damage, projectile paths and impact effects are unchanged.

Single-client Studio validation: Glock produced four flashes from four launch packets, Draco ten from ten, and shotgun one flash from five simultaneous pellet launch packets. All three variants observed. Final pooled Draco pass saw ten distinct shots and all three variants; 40 of 43 sampled active frames reported ImageLabel.IsLoaded (initial rendering can still need a frame). Barrel-follow sampling found zero position error across 36 frames. Selected sprites were visually inspected at review size and a small yellow muzzle flash captured during actual firing. Expiry hides pooled images; no visible effects remained after disabling weapons. Earlier muzzle tests had no gameplay console errors. A later console read reported an unrelated ShopLayoutQA LocalScript missing RunClientScript capability; it was left untouched. The final shotgun follow-up emitted zero shot packets, so that follow-up is inconclusive; the earlier five-pellets/one-flash result remains the completed shotgun check. Both Rojo packages built successfully. Multiple real clients, published asset permissions and latency remain untested. Temporary test camera/UI/position changes were discarded on returning to Edit.
