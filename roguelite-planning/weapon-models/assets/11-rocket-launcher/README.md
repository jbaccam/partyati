...
Rear bore correction (September 22, 2026): the original dark rear closure shared X=-2.55 with the green tube closure. The dark disk is now recessed to X=-2.52, eliminating their coplanar overlap while keeping the green inset. `batches/guns/fix_launcher_rear_cap.py` reproduces the repair without changing the atlas; original exports are in `backups/20260922-rear-cap`. The imported Roblox mesh is retained and uses `WeaponPresentation` version 2 native rear-cap covers until these corrected exports are reimported.
