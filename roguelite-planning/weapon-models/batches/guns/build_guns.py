...
def rocket():
    ...
    rear=barrel('Rear flared shoulder opening',-2.85,-2.45,.75,CHAR,2.10)
    # The blind dark disk used to share x=-2.55 with the olive tube end.
    # Recess it .03 behind that disk so camera movement cannot cause z-fighting.
    for v in rear.data.vertices:
        if abs(v.co.x+2.55)<1e-5:v.co.x+=.03
