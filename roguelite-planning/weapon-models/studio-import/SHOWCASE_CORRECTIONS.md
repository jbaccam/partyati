...
- Bowling ball now uses uniform black RGB 17,17,17, including the real finger sockets. Supersedes the gray body and painted dark socket atlas (114276616497392). Studio uses no TextureID and SmoothPlastic, so socket shading comes from lighting.
...
September 21 bowling correction: `ApplyBowlingBlack.luau` updates both existing display sets and `ReplicatedStorage.RogueliteCombat.WeaponTemplates.22`, preserving geometry/transforms. The authored bowling model generator now assigns the same black color to every face. `fix_bowling_black.py` updates the current full corrected Blender/FBX derivative; run it after `fix_magic_materials.py` when rebuilding that derivative. Studio Edit close-up visually checked; all three active meshes use the same texture-free black material. No gameplay logic changed.
